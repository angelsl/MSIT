...
namespace MSIT.WzLib.WzProperties
{
    ...
    /// </summary>
    public class WzSoundProperty : Extended
    {
        #region Fields
        ...
        public WzSoundProperty(string name, WzBinaryReader reader, bool parseNow)
        {
            ...
            if (parseNow) mp3bytes = reader.ReadBytes(soundDataLen);
            else reader.BaseStream.Position += soundDataLen;
        }
        ...
        {
            if (mp3bytes != null) return mp3bytes;
            else
            {
                ...
                if (saveInMemory) return mp3bytes;
                else
                {
